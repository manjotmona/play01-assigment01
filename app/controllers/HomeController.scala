...
import models.form.{UserLoginData, UserLoginForm, UserSignupData, UserSignupForm, forgetPasswordForm}
        ...
        userData => {
          ...
          userSignupRepo.updateUserProfile(user).flatMap {
            ...
            case false => Logger.info("not updated")
              Future
                .successful(Redirect(routes.HomeController.login()).flashing("status" -> "Could not update"))
          ...
          user.flatMap {
            case Some(loginUser) => Logger.info("Some user")
              if (loginUser.password == userLoginPageData.password) {
                if (loginUser.isAdmin == false && loginUser.isEnabled == true) {
                  Future
                    .successful(Redirect(routes.HomeController.profile())
